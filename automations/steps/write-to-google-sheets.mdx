...
**Result:** AffZero finds the row where Affiliate is "Acme Corp" AND Month is "March 2026", and fills in Revenue and Clicks. The rest of the sheet is untouched. The next run of this automation (April 1st) would find and fill the April 2026 row.

### Notes on matching

- Matching is **case-insensitive** and trims surrounding whitespace.
- If there are **multiple rows** that match all conditions, only the **first** one is filled.
- Variable values in match conditions are resolved at run time — so `{date_month} {date_year}` becomes `"March 2026"` when the automation runs in March.
...
**Date variables** are particularly useful for match conditions and append values:

| Variable | Example value | Description |
|----------|---------------|-------------|
| `{date_month_year}` | March 2026 | Month and year (long form) |
| `{date_month}` | March | Month name |
| `{date_year}` | 2026 | Year |
| `{date_long_format}` | March 15, 2026 | Full date |
| `{date_start_day}` | Mar 1 | Start of date range |
| `{date_end_day}` | Mar 31 | End of date range |
...
Use **Match & Fill**. Set the match conditions to identify the current month's row (e.g. Affiliate = your affiliate name AND Month = `{date_month_year}`). Set the fill mappings to the columns you want to populate. Each month's automation run will find and fill the correct row.
